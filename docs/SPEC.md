...
| 状態 | 外付けリング | 内蔵LED |
|------|------|------|
| 設定モード（AP待ち） | シアンのゆっくり呼吸 | シアン呼吸 |
| WiFi接続中 | 黄色のコメットが回転 | 黄色点滅 |
| 通常・雨 | 青の回転アニメーション | 青点滅 |
| 通常・晴れ | オレンジのゆっくり明滅 | オレンジ呼吸 |
| リセット長押し中 | 赤の進捗バー（点灯数が増える） | 赤点滅（進むほど速く） |
| リセット確定 | 赤の速い点滅 | 赤の速い点滅 |

> 本体ボタンを長押しすると、押している間リングが赤で埋まっていき（進捗）、`RESET_HOLD_MS`到達で
> 赤点滅→WiFi設定リセット＆再起動。途中で離せばキャンセル。
