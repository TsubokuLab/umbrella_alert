...
- 起動後、APモード（SSID: `Umbrella-Alert-XXXX` / PASS: `12345678`）。本体LCDにQR/URL（`http://192.168.4.1`）が出る
- スマホで接続すると**1画面のキャプティブポータル**が直接WiFi設定フォームを表示 → WiFiを登録 → 再起動して接続
- **場所の設定**：スマホを自宅Wi-Fiに戻し `http://umbrella-alert-xxxx.local`（状態ページ）を開く
  → **47都道府県のプルダウン**で選ぶ、または「🗺 地図から指定する」で任意座標（GitHub Pages地図）。保存は即時反映（再起動なし）
  - 本体LCDの「設定画面」に出るQR（**設定画面URL**）からも、この本体ページを開けます
  - 本体LCDのタッチ操作で従来の5都市プリセットからも選べます（ハイブリッド）
...
2. スマホでAPに接続 → **1画面のキャプティブポータル**で **WiFiを保存** → 自動再起動
   （※IPは再起動後にしか決まらないため、場所の設定は接続後に行う。完了画面の本体URLをコピーしておくと便利）
3. 接続中はLED=**黄色のコメットが回転**。接続成功で通常動作（雨=青回転／晴=オレンジ明滅）
4. **場所の設定**：スマホを**自宅Wi-Fiに繋ぎ直して** `http://umbrella-alert-xxxx.local`（状態ページ）を開き、
   **47都道府県のプルダウン**から選ぶ → 保存で即時反映（再起動なし）
   - もっと細かく指定したい場合は「🗺 地図から指定する」→ GitHub Pages地図ページで任意座標を指定
5. **WiFiをやり直す**：本体ボタンを**3秒長押し**で設定をリセット → 設定モードに戻る
   - 押している間はLEDが**赤点滅**、到達すると**赤を1秒点灯→消灯→リセット**。途中で離せばキャンセル
